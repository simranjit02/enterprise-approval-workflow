...
service ApprovalService
{
    ...
    [
        { grant : [ 'READ' ], to : [ 'Requester', 'Manager', 'Finance', 'Admin' ] }
    ];
    ...
    @odata.draft.enabled
    entity Requests as
        projection on db.PurchaseRequest
        actions
        {
            @restrict :
            [
    ...
    entity RequestItems as
        ...
        actions
        {
            ...
            returns RequestItems;
        };

    entity ApprovalSteps as
        projection on db.ApprovalStep
        order by stepNumber asc;

    entity AuditLogs as
        projection on db.AuditLog;
...
annotate ApprovalService with @requires :
[
    'authenticated-user'
];
